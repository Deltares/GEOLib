...
def calculate_fragility_curve(
    ...
    :i = 1   #counter of iteration
    :n = 5  # maximum iteration
    """
    ...
    # Here we should add the Bisection to find the next choosing Length

    #We should add the condition here to check the next steps. 
    condition = True
    i = 1
    for i in range(n):
        #while condition:
        ...
        new_length = (length_start + length_end) / 2
        ...
        print("End: " + str(length_end))
        print("Start: " + str(length_start))
        ...
        if result.ReliabilityIndex < TargetBeta:    
            length_start = new_length 
        else:    
            length_end = new_length
        
        if tolerance is not None and abs(TargetBeta - result.ReliabilityIndex) < tolerance:
            break
        # if i == n:
        #   condition = False
        # else:
...
if __name__ == "__main__":
    ...
    df = calculate_fragility_curve(
        input_file, fixed_berm_points, varying_berm_points, z_start, z_end, n, TargetBeta, tolerance
    )
