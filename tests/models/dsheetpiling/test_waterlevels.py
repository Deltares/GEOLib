...
from geolib.models.dsheetpiling.internal import WaterLevels
...
def make_water_level() -> WaterLevel:
    def _make_water_level(name: str):
        return WaterLevel(name=name, level=-1)
...
class TestWaterLevels:
    ...
    def test_dsheetpilingmodel_add_water_level_invalid_stage_id_raises_ValueError(
        self, _model: DSheetPilingModel, make_water_level: Callable
    ):
        ...
        with pytest.raises(
            ...
            _model.add_head_line(
                water_level=water_level, side=Side.BOTH, stage_id=invalid_stage_id
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                Side.LEFT, "Left water_level", _DEFAULT_WATER_LEVEL_NAME, id="Left side"
            ),
            pytest.param(
                Side.RIGHT,
            ...
            pytest.param(
                Side.BOTH, "Same water_level", "Same water_level", id="Both sides"
            ),
        ],
    )
    def test_dsheetpilingmodel_add_water_level_internal_construction_stages(
        ...
        if side == Side.LEFT:
            ...
            assert (
                left_water_level_name
                == right_water_level_name
                != _DEFAULT_WATER_LEVEL_NAME
            )
            water_level = make_water_level(right_water_level_name)

        _model.add_head_line(
            water_level=water_level, side=side, stage_id=current_stage_id
        )

        current_stage = _model.datastructure.input_data.construction_stages.stages[
            current_stage_id
        ]
