...
from geolib.models.dsheetpiling.internal import (
    _DEFAULT_SURFACE_NAME,
    Surfaces,
    Surface as InternalSurface,
)
...
def make_surface() -> Surface:
    def _make_surface(name: str):
        return Surface(
            name=name,
            points=[Point(x=0, z=0), Point(x=1, z=0)]
        )
...
class TestSurfaces:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param([Point(x=0, z=0), Point(x=1, z=0)], does_not_raise(), id="Multiple points"),
            pytest.param([], pytest.raises(ValidationError, match=r"ensure this value has at least 1 items"), id="No points"),
            pytest.param([Point(x=-1, z=0)], pytest.raises(ValidationError, match=r"All x-coordinates must be greater than or equal to 0, found"), id="Negative x-coordinates"),
            pytest.param([Point(x=1, z=0)], pytest.raises(ValidationError, match=r"X-coordinate first point should be zero, received"), id="First x-coordinate not zero"),
            pytest.param([Point(x=0, z=0), Point(x=10, z=0), Point(x=5, z=0)], pytest.raises(ValidationError, match=r"x-coordinates must be strictly increasing"), id="Not strictly increasing"),
        ],
    )
    def test_surface_initialization_with_different_points_arguments(self, points: List[Point], run_expectation):
    ...
    def test_dsheetpilingmodel_add_surface_internal_surfaces_updated(self, _model: DSheetPilingModel, make_surface: Callable, side: Side):
        ...
        for i, (point, internal_point) in enumerate(zip(surface.points, internal.points), start=1):
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(Side.LEFT, "Left surface", _DEFAULT_SURFACE_NAME, id="Left side"),
            pytest.param(Side.RIGHT, _DEFAULT_SURFACE_NAME, "Right surface", id="Right side"),
