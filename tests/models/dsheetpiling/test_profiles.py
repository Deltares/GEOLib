...
from geolib.models.dsheetpiling.internal import (
    ...
    SoilProfiles,
    SoilProfile as InternalProfile,
)
...
def make_profile() -> SoilProfile:
    def _make_profile(name: str):
        return SoilProfile(
            name=name,
            layers=_TEST_LAYERS,
            coordinates=_PROFILE_TEST_COORDINATES
        )
...
class TestCurveSettings:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(0, pytest.raises(ValidationError, match=r"ensure this value is greater than or equal to 1"), id="Lower than allowed"),
            pytest.param(1, does_not_raise()),
            pytest.param(2, does_not_raise()),
            pytest.param(3, does_not_raise()),
            pytest.param(4, does_not_raise()),
            pytest.param(5, pytest.raises(ValidationError, match=r"ensure this value is less than or equal to 4"), id="Higher than allowed"),
        ]
    )
    ...
    def test_dsheetpilingmodel_add_curve_settings_internal_soil_profiles_updated(
        ...
        assert _model.datastructure.input_data.soil_profiles.modulus_reaction_type == modulus_reaction_type.value
        assert _model.datastructure.input_data.soil_profiles.use_unloading_reloading_curve == use_unloading_reloading_curve
...
class TestSoilProfile:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                [],
                pytest.raises(ValidationError, match=r"ensure this value has at least 1 items"),
                id="No layers"
            ),
            pytest.param(
                [
                ...
                pytest.raises(ValidationError, match=r"Top of layer must decrease with each layer along depth. Top of layers: "),
                id="Multiple layers, top of layer not decreasing"
            ),
        ],
    )
    def test_profile_initialization_with_different_points_arguments(self, layers: List[SoilLayer], raise_context):
        with raise_context:
    ...
    @pytest.mark.integrationtest
    def test_dsheetpilingmodel_add_profile_soil_unknown_raises_ValueError(self, _model: DSheetPilingModel):
        ...
        assert name_not_in_soil_collection not in _model.datastructure.input_data.soil_collection.soil_names
    ...
    def test_dsheetpilingmodel_add_profile_internal_soil_profiles_updated(self, _model: DSheetPilingModel, make_profile: Callable, side: Side):
        ...
        internal = _model.datastructure.input_data.soil_profiles.soil_profiles[current_stage]
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(Side.LEFT, "Left profile", _DEFAULT_SOIL_PROFILE_NAME, id="Left side"),
            pytest.param(Side.RIGHT, _DEFAULT_SOIL_PROFILE_NAME, "Right profile", id="Right side"),
    ...
    def test_dsheetpilingmodel_add_profile_internal_construction_stages(
        ...
        assert _model.datastructure.input_data.construction_stages.stages[current_stage].soil_profile_left == left_profile_name
        assert _model.datastructure.input_data.construction_stages.stages[current_stage].soil_profile_right == right_profile_name
