...
from geolib.models.dstability.internal import PersistablePoint, PersistableHeadLine, PersistableReferenceLine, Waternet
...
class TestDStabilityHeadLine:
    ...
    def test_add_head_line(self):
        ...
        headline_id = dsm.add_head_line(label="TestHL", points=points, is_phreatic_line=True)
...
class TestDStabilityReferenceLine:
    ...
    def test_add(self):
        ...
        points = [Point(x=-20.0, z=-2.0), Point(x=50.0, z=-2.0)]
        head_line_1_id = dsm.add_head_line(label="TestHL_1", points=points, is_phreatic_line=True)
        head_line_2_id = dsm.add_head_line(label="TestHL_2", points=points, is_phreatic_line=False)
        ...
        reference_line = dsm.datastructure.waternets[0].get_reference_line(str(reference_line_id))
