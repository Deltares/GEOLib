...
def _get_dstability_model():
    ...
    test_cases = [
        (AnalysisType.UPLIFT_VAN, CalculationType.DETERMINISTIC, 'uplift_van_results', _uplift_van_result),
        (AnalysisType.UPLIFT_VAN, CalculationType.PROBABILISTIC, 'uplift_van_reliability_results', _uplift_van_reliability_result),
        (AnalysisType.UPLIFT_VAN_PARTICLE_SWARM, CalculationType.DETERMINISTIC, 'uplift_van_particle_swarm_results', _uplift_van_particle_swarm_result),
        (AnalysisType.SPENCER, CalculationType.DETERMINISTIC, 'spencer_results', _spencer_result),
        (AnalysisType.SPENCER, CalculationType.PROBABILISTIC, 'spencer_reliability_results', _spencer_reliability_result),
        (AnalysisType.SPENCER_GENETIC, CalculationType.DETERMINISTIC, 'spencer_genetic_algorithm_results', _spencer_genetic_algorithm_result),
        (AnalysisType.BISHOP, CalculationType.DETERMINISTIC, 'bishop_results', _bishop_result),
        (AnalysisType.BISHOP, CalculationType.PROBABILISTIC, 'bishop_reliability_results', _bishop_reliability_result),
        (AnalysisType.BISHOP_BRUTE_FORCE, CalculationType.DETERMINISTIC, 'bishop_bruteforce_results', _bishop_bruce_force_result),
    ]
    ...
    for i, (analysis_type, calculation_type, result_attribute, result_class) in enumerate(test_cases):
        ...
        getattr(model.datastructure, result_attribute).append(result_class(result_id=result_id))
...
class TestDStabilityResults:
    ...
    def test_get_slipcircle_result(self, _get_dstability_model):
        ...
        for i, _ in enumerate(model.datastructure.stages):
            ...
            if isinstance(result_substructure, (SpencerReliabilityResult, SpencerResult, SpencerGeneticAlgorithmResult)):
                ...
                assert isinstance(slipplane_result, (BishopSlipCircleResult, UpliftVanSlipCircleResult))
    ...
    def test_get_slipplane_result(self, _get_dstability_model):
        ...
        for i, _ in enumerate(model.datastructure.stages):
            ...
            if not isinstance(result_substructure, (SpencerReliabilityResult, SpencerResult, SpencerGeneticAlgorithmResult)):
                with pytest.raises(AttributeError):
